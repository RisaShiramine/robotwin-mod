...
class place_mousepad2(Base_Task):
    ...
    def _iter_render_bodies(self, actor: Actor):
        entity = actor.actor if hasattr(actor, "actor") else actor
        if not hasattr(entity, "get_components"):
            return
    ...
    def _iter_render_shapes(self, actor: Actor):
        for component in self._iter_render_bodies(actor):
    ...
    def _set_actor_rgba(self, actor: Actor, rgba: list[float]):
        visible = rgba[3] > 0.01
        ...
        for shape in self._iter_render_shapes(actor):
            material = None
            ...
            if hasattr(material, "base_color"):
                ...
                material.set_base_color(rgba)

            if hasattr(shape, "set_material"):
                shape.set_material(material)

    def _set_mousepad_transparent(self, transparent: bool):
        alpha = 0.0 if transparent else 1.0
        rgb = self.color_value
        self._set_actor_rgba(self.target, [rgb[0], rgb[1], rgb[2], alpha])

    def play_once(self):
        arm_tag = ArmTag("right" if self.mouse.get_pose().p[0] > 0 else "left")

        # Make mouse pad transparent while grasping the mouse
        self._set_mousepad_transparent(True)
        try:
            self.move(self.grasp_actor(self.mouse, arm_tag=arm_tag, pre_grasp_dis=0.1))
        finally:
            # Recover mouse pad after the mouse is grasped (or if grasp fails)
            self._set_mousepad_transparent(False)
