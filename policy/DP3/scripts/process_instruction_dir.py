...
def choose_instruction(payload: Dict[str, List[str]], source: str) -> str:
...
def collect_instruction_dir(instruction_dir: str, instruction_source: str) -> Dict[str, Any]:
    ...
    for json_file in sorted(root.glob("episode*.json")):
        ...
        episode_map[str(episode_id)] = {
            "instruction": choose_instruction(payload, instruction_source),
...
def main() -> None:
    ...
    if args.decompose:
        ...
        decompositions = decompose_aggregated(collected["aggregated"], args)
